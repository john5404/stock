"""Shared yfinance OHLCV download and local cache."""
...
def update_yf_cache(
    ...
    *,
    ...
    label = market_label or ticker

    cached = load_ohlcv_cache(path)

    if cached is None:
        fresh = download_range(ticker, need_from, as_of)
        if fresh.empty:
            fresh = download_period(ticker, period)
        if fresh.empty:
            raise ValueError(f"No data returned for ticker: {label}")
...
def get_yf_stock_data(
    ...
    *,
    ...
    cached = update_yf_cache(
        ticker,
        period,
        data_dir=data_dir,
        lookback_days=lookback_days,
        as_of=as_of,
        market_label=market_label or ticker,
    )
